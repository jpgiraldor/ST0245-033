...
public class Recursion2CB
{
    ...
    return groupSum6(start+1, nums, target-nums[start]) || groupSum6(start +1, nums, target);  //T(n-1)+T(n)
  }     //
  ...
  public boolean groupSum5(int start, int[] nums, int target) {                         //c1
    ...
    return groupSum5(start+1,nums,target-nums[start]) || (nums[start]%5!=0 && groupSum5(start+1,nums,target)); //T(n-1)+T(n)
   }    //
  ...
  public boolean groupNoAdj(int start, int[] nums, int target) {                                            //c1
      ...
      return groupNoAdj(start + 1, nums, target) || groupNoAdj(start + 2, nums, target - nums[start]);    //T(n-1)+T(n)
  }
